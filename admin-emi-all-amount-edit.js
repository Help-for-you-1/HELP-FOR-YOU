/* HELP FOR YOU — ADMIN EMI ALL AMOUNT EDIT ONLY
   Adds Admin-only editing for every editable amount shown in EMI schedule.
   Does not alter Staff/Customer payment flow.
   total_due and remaining_amount are generated columns and must NOT be updated directly.
*/
(function(){
'use strict';
...
window.hfyEditEmi=async function(id){
 try{
  ...
  openBox('Edit EMI — All Amounts','<div class="form">'+
   ...
   '<label>Total Due<input id="hfyaeTotal" type="number" value="'+total.toFixed(2)+'" readonly></label>'+ 
   '<label>Paid Amount<input id="hfyaePaid" type="number" min="0" step="0.01" value="'+paid.toFixed(2)+'"></label>'+ 
   '<label>Remaining Amount<input id="hfyaeRemain" type="number" value="'+remaining.toFixed(2)+'" readonly></label>'+ 
