...
function overduePenalty(e){const due=String(e.due_date||'').slice(0,10),t=today();if(!due||due>=t||String(e.status||'').toLowerCase()==='paid')return Number(e.penalty||0);const days=Math.max(0,Math.floor((new Date(t+'T00:00:00')-new Date(due+'T00:00:00'))/86400000));const amount=Number(e.emi_amount||0);return Number((amount*(Math.pow(1.05,days)-1)).toFixed(2));}
function status(e){let s=String(e.status||'pending').toLowerCase();if(s==='paid')return 'paid';const p=overduePenalty(e),r=Math.max(0,Number(e.emi_amount||0)+p-Number(e.paid_amount||0));if(r<=0)return 'paid';return String(e.due_date||'')<today()?'overdue':'pending';}
...
async function viewRepayFix(i){try{
 ...
 const rows=es.map(e=>{const s=status(e),a=Number(e.emi_amount||0),p=overduePenalty(e),pa=Number(e.paid_amount||0),td=a+p,r=Math.max(0,td-pa);total+=td;paid+=pa;remaining+=r;penalty+=p;if(s==='paid')paidCount++;else if(s==='overdue')overdueCount++;else pendingCount++;const action=s==='paid'?'<span class="paid"><b>Paid</b></span>':'<button class="btn '+(s==='overdue'?'red':'green')+'" onclick="hfyPay(\''+esc(e.id)+'\')">Pay Now</button>';return '<tr><td>'+esc(e.emi_number)+'</td><td>'+esc(String(e.due_date||'').slice(0,10))+'</td><td>₹'+money(a)+'</td><td>₹'+money(p)+'</td><td>₹'+money(td)+'</td><td>₹'+money(pa)+'</td><td>₹'+money(r)+'</td><td class="'+(s==='paid'?'paid':s==='overdue'?'over':'pending')+'"><b>'+s+'</b></td><td>'+action+' <button class="btn gray" onclick="hfyEditEmi(\''+esc(e.id)+'\')">Edit</button></td></tr>';}).join('')||'<tr><td colspan="9">No EMI schedule found.</td></tr>';
