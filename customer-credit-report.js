...
function profile(c){
 ...
 const historicalOverdue=emis.filter(e=>Number(e.penalty||0)>0).length;
 const overdueCount=Math.max(currentOverdue.length,historicalOverdue);
 ...
 return {loans,emis,paid,overdue,closed,active,totalDue,totalPaid,onTime,lateDays,repaymentRate,lateRate,score,dueEmis,futureEmis,duePaid,currentOverdue,overdueCount,historicalOverdue,amountRate,maxOverdueDays,latePaymentRows,closedRate};
}
