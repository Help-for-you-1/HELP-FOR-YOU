...
const pick=(...z)=>z.find(q=>q!==undefined&&q!==null&&String(q)!=='')||'';
const isClosed=x=>['closed','completed','fully_paid','paid_off'].includes(String(pick(x.loan_status,x.account_status,x.status)).toLowerCase());
async function getClosedLoans(){
 const sb=db();
 ...
 const apps=a.data||[], customers=c.data||[], loans=l.data||[];
 const result=[];
 loans.filter(isClosed).forEach(loan=>{
  const app=apps.find(x=>String(x.loan_id||'')===String(loan.loan_id||'')&&String(x.customer_id)===String(loan.customer_id))||apps.find(x=>String(x.customer_id)===String(loan.customer_id)&&String(x.status||'').toLowerCase()==='approved')||{};
  const customer=customers.find(x=>String(x.id)===String(loan.customer_id))||{};
  result.push({loan,app,customer});
 });
 return result;
}
function addButton(){
...
window.hfyOpenNOC=async function(){
 try{
  const list=await getClosedLoans();
  if(!list.length)return alert('No closed loan account found. NOC is available only for closed loans.');
  const options=list.map((x,i)=>`<option value="${i}">${esc(pick(x.customer.full_name,x.app.full_name,x.app.name,'Customer'))} — Loan ID ${esc(pick(x.loan.loan_id,x.app.loan_id,'N/A'))}</option>`).join('');
  const html=`<h2>No Objection Certificate</h2><p>Only closed loan accounts are eligible for NOC. Each loan is shown separately.</p><label>Closed Loan<select id="hfyNocSelect" onchange="hfyFillNOC()">${options}</select></label><div id="hfyNocForm"></div><div style="margin-top:14px"><button class="btn green" onclick="hfyPrintNOC()">🖨️ Print NOC</button></div>`;
