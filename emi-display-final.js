...
window.viewLoanEmi=async function(i){
 try{
  ...
  const es=await window.ensure30(loan);
  ...
  const html=es.map(e=>{
   ...
   const s=window.status30(e); paidTotal+=pa;penalty+=p;if(s==='paid')paidCount++;else if(s==='overdue')overdue++;else pending++;
   const totalDueBalance=Math.max(0,loanTotal-paidTotal);
   return `<tr><td>${escD(e.emi_number)}</td><td>${escD(String(e.due_date||'').slice(0,10))}</td><td>${moneyD(a)}</td><td>${moneyD(p)}</td><td>${moneyD(totalDueBalance)}</td><td>${moneyD(pa)}</td><td>${moneyD(remaining)}</td><td class="${s==='paid'?'paid':s==='overdue'?'over':'pending'}">${escD(s)}</td><td><button class="btn green" onclick="emi30Paid('${escD(e.id)}')">Paid</button></td></tr>`;
  }).join('');
  const balance=Math.max(0,loanTotal-paidTotal);
  window.openBox('Full EMI List',`<p><b>Name:</b> ${escD(c.data?.full_name||'-')} &nbsp; <b>Loan ID:</b> ${escD(loan.loan_id||'-')} &nbsp; <b>Mobile:</b> ${escD(c.data?.mobile||'-')} &nbsp; <b>Sanction Loan:</b> ${moneyD(loan.loan_amount)} &nbsp; <b>Total Loan:</b> ${moneyD(loanTotal)} &nbsp; <b>Sanction Date:</b> ${escD(loan.start_date||'-')}</p><div class="wrap"><table style="min-width:1200px"><thead><tr><th>EMI No.</th><th>Due Date</th><th>EMI Amount</th><th>Penalty</th><th>Total Due</th><th>Paid</th><th>Remaining</th><th>Status</th><th>Action</th></tr></thead><tbody>${html}</tbody></table></div><p><b>Total EMI:</b> ${es.length} &nbsp; <b>Paid:</b> ${paidCount} &nbsp; <b>Pending:</b> ${pending} &nbsp; <b>Overdue:</b> ${overdue} &nbsp; <b>Total Paid:</b> ${moneyD(paidTotal)} &nbsp; <b>Remaining:</b> ${moneyD(balance)} &nbsp; <b>Penalty:</b> ${moneyD(penalty)} &nbsp; <b>Total Due:</b> ${moneyD(balance)}</p>`);
