...
const overduePenaltyD=e=>{
 const due=String(e.due_date||'').slice(0,10),today=new Date().toISOString().slice(0,10);
 if(!due||due>=today||String(e.status||'').toLowerCase()==='paid')return Number(e.penalty||0);
 const days=Math.max(0,Math.floor((new Date(today+'T00:00:00')-new Date(due+'T00:00:00'))/86400000));
 const amount=Number(e.emi_amount||0);
 return Number((amount*(Math.pow(1.05,days)-1)).toFixed(2));
};
...
window.viewLoanEmi=async function(i){
 try{
  ...
  let loanTotal=0,paidTotal=0,penalty=0,paidCount=0,pending=0,overdue=0;
  const html=es.map(e=>{
   const a=Number(e.emi_amount||0),p=overduePenaltyD(e),pa=Number(e.paid_amount||0),emiDue=a+p,remaining=Math.max(0,emiDue-pa);
   ...
   loanTotal+=emiDue;paidTotal+=pa;penalty+=p;if(s==='paid')paidCount++;else if(s==='overdue')overdue++;else pending++;
   ...
   const edit=`<button class="btn" onclick="hfyEditEmi('${escD(e.id)}')" style="margin-left:4px">Edit</button>`;
   return `<tr><td>${escD(e.emi_number)}</td><td>${escD(String(e.due_date||'').slice(0,10))}</td><td>${moneyD(a)}</td><td>${moneyD(p)}</td><td>${moneyD(emiDue)}</td><td>${moneyD(pa)}</td><td>${moneyD(remaining)}</td><td class="${s==='paid'?'paid':s==='overdue'?'over':'pending'}">${escD(s)}</td><td>${action}${edit}</td></tr>`;
  }).join('');
