...
function P(e){var due=String(e.due_date||'').slice(0,10),today=new Date().toISOString().slice(0,10);if(!due||due>=today||String(e.status||'').toLowerCase()==='paid')return +e.penalty||0;var days=Math.max(0,Math.floor((new Date(today+'T00:00:00')-new Date(due+'T00:00:00'))/86400000));var base=Number(e.emi_amount||0);return +(base*Math.pow(1.05,days)-base).toFixed(2)}
function S(e){var p=P(e),r=Math.max(0,(+e.emi_amount||0)+p-(+e.paid_amount||0));if(String(e.status||'').toLowerCase()==='paid'||r<=0)return'paid';return String(e.due_date||'')<new Date().toISOString().slice(0,10)?'overdue':'pending'}
...
function showList(c,es){
 ...
 es.forEach(function(e){var s=S(e),a=+e.emi_amount||0,p=P(e),pa=+e.paid_amount||0,r=Math.max(0,a+p-pa);total+=a+p;paidAmt+=pa;remaining+=r;penalty+=p;if(s==='paid')paid++;else if(s==='overdue')overdue++;else pending++});
 var rows=es.map(function(e){var s=S(e),a=+e.emi_amount||0,p=P(e),pa=+e.paid_amount||0,r=Math.max(0,a+p-pa);return '<tr><td>'+E(e.emi_number)+'</td><td>'+E(String(e.due_date||'').slice(0,10))+'</td><td>₹'+M(a)+'</td><td>₹'+M(p)+'</td><td>₹'+M(a+p)+'</td><td>₹'+M(pa)+'</td><td>₹'+M(r)+'</td><td class="'+(s==='paid'?'paid':s==='overdue'?'over':'pending')+'"><b>'+s+'</b></td><td>'+(s==='paid'?'<span class="paid"><b>Paid</b></span>':'<button class="btn '+(s==='overdue'?'red':'green')+'" onclick="hfyPay(\''+E(e.id)+'\')">Paid</button>')+' <button class="btn gray" onclick="hfyEditEmi(\''+E(e.id)+'\')">Edit</button></td></tr>'}).join('');
