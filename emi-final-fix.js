...
function P(e){var due=String(e.due_date||'').slice(0,10),today=new Date().toISOString().slice(0,10);if(!due||due>=today||String(e.status||'').toLowerCase()==='paid')return +e.penalty||0;var days=Math.max(0,Math.floor((new Date(today+'T00:00:00')-new Date(due+'T00:00:00'))/86400000));var base=Number(e.emi_amount||0);return +(base*(Math.pow(1.05,days)-1)).toFixed(2)}
