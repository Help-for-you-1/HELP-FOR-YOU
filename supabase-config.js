  ...
  if (location.pathname.toLowerCase().endsWith('/admin.html')) {
    ...
    setTimeout(function fixApprovalIndex() {
      try {
        ...
        if (pending.length) {
          window.apRows.innerHTML = pending.map(({ x, index }) => `<tr><td>${esc(x.name)}</td><td>${esc(x.mobile)}</td><td>${esc(x.date)}</td><td>₹${money(x.amount)}</td><td class="pending">Pending for Approval</td><td><button class="btn blue" onclick="editApproval(${index})">Edit</button></td></tr>`).join('');
        }
...
(function () {
  function bootAddCustomer() {
    ...
    window.openAddCustomer = function () {
      ...
      openBox('Add Customer', `<div class="form">
        ...
        <div class="full"><button class="btn green" onclick="saveNewCustomer()">Save Customer</button><button class="btn gray" onclick="closeM()">Cancel</button></div>
      </div>`);
    };
...
// A newly submitted application has status "pending" and will therefore appear automatically.
