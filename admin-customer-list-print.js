...
function printList(title,headers,rows){
 ...
 const logoUrl=new URL('hfy-logo.svg',location.href).href;
 w.document.write('<!doctype html><html><head><meta charset="utf-8"><title>HELP FOR YOU | '+esc(title)+'</title><style>body{font:12px Arial;color:#172033;margin:0}header{position:relative;border-bottom:3px solid #123f7a;padding:12px 12px 12px 125px;min-height:90px;margin-bottom:18px;box-sizing:border-box;display:flex;flex-direction:column;justify-content:center;text-align:center}.hfy-print-logo{position:absolute;left:18px;top:8px;width:90px;height:90px;display:flex;align-items:center;justify-content:center;pointer-events:none}.hfy-print-logo img{width:90px;height:90px;object-fit:contain;display:block}h1{font-size:22px;color:#123f7a;margin:0 0 5px}h2{font-size:15px;margin:0;color:#475569}.meta{margin:10px 0 0;color:#64748b}.meta b{font-weight:600}table{width:100%;border-collapse:collapse}th,td{border:1px solid #cbd5e1;padding:8px;text-align:left}th{background:#eef3f8;color:#123f7a}.sign{margin-top:55px;display:flex;justify-content:space-between}.line{border-top:1px solid #172033;width:220px;text-align:center;padding-top:6px}@page{size:A4;margin:14mm}</style></head><body><header><div id="hfy-print-logo" class="hfy-print-logo"><img src="'+logoUrl+'" alt="HELP FOR YOU"></div><h1>HELP FOR YOU</h1><h2>'+esc(title)+'</h2><div class="meta">Generated: <b>'+new Date().toLocaleDateString('en-IN')+'</b></div></header><table><thead><tr>'+headers.map(h=>'<th>'+esc(h)+'</th>').join('')+'</tr></thead><tbody>'+(rows.length?rows.join(''):'<tr><td colspan="'+headers.length+'" style="text-align:center">No records found</td></tr>')+'</tbody></table><div class="sign"><div class="line">Prepared By</div><div class="line">Authorised Signatory</div></div></body></html>');w.document.close();w.focus();setTimeout(()=>w.print(),300);
}
